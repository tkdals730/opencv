...
# 번호판 이미지 가져오기 
img = np.fromfile('01가1134.jpg', dtype=np.uint8)
...
# 3단계 : 적응형 임계처리
binary = cv2.adaptiveThreshold(contrast, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 41, 25)
...
text = pytesseract.image_to_string(filtered, lang='kor', config=config)
print(f"인식 결과: {text}")
